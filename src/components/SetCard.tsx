...
export const SetCard = ({
    ...
    const isNewSet = set.id.startsWith("temp-");

    return (
        <Card 
            id={`set-${set.id}`} // For auto-scrolling
            className={cn(
                "overflow-hidden border-2 transition-all duration-500",
                isEncore ? "border-amber-400 bg-amber-50/30 dark:bg-amber-950/10" : "",
                isNewSet ? "border-green-500/50 bg-green-50/10 shadow-[0_0_15px_rgba(34,197,94,0.1)]" : ""
            )}
        >
            <CardHeader 
                className={cn(
            ...
            >
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <Badge 
                        variant={isEncore ? "default" : "outline"} 
                        className={cn(
                            "text-base px-3 py-1 whitespace-nowrap shrink-0 transition-colors", 
                            isEncore ? "bg-amber-500 hover:bg-amber-600" : "bg-background",
                            isNewSet && !isEncore ? "border-green-500 text-green-700 dark:text-green-400" : ""
                        )}
                    >
                        {isEncore && <Star className="w-3 h-3 mr-1 fill-current" />}
                        {set.name}
                        {isNewSet && <span className="ml-2 text-[10px] uppercase font-bold tracking-wider opacity-70">(New)</span>}
                    </Badge>
                    ...
                    {set.songs.length === 0 ? (
                        ...
                        <div className="divide-y">
                            {set.songs.map((setSong, index) => {
                                const isNewSong = setSong.id.startsWith("temp-");
                                return (
                                    <div 
                                        key={setSong.id} 
                                        className={cn(
                                            "flex items-center p-3 transition-colors group",
                                            isNewSong ? "bg-green-50/30 dark:bg-green-900/10 hover:bg-green-100/40" : "hover:bg-accent/30"
                                        )}
                                    >
